...
from feishu.file.doc_parser import DocParser, Person, ImageItem, File, DocItem
...
class Doc(BaseFile):
    ...
    def refresh(self):
        data = doc_meta(self.drive.login.auth, self.token)
        if data:
    ...
    def get_instance(drive, token, folder_token=None):
        ...
        data = doc_meta(drive.login.auth, token)
        if data is None:
            return None
    ...
    def get_content(self):
        data = doc_content(self.drive.login.auth, self.token)
        if data is None:
            return None
        return data.content
    ...
    def upload(drive, folder_token, content):
        if folder_token not in drive.all_file_map:
            ...
            return None
        data = doc_create(drive.login.auth, folder_token, content)
        if data is None:
            return None
